...
namespace TechEvent
{
    public static class TechEventHelper
    {
        ...
        public static string XMLDataOlustur()
        {
            ...
            foreach (Etkinlik item in etkinlikListesi)
            {
                xml += @$"<Etkinlik>
                            ...
                            <KisiSayisi>{item.Kontenjan}</KisiSayisi>
        ...
        public static string JSONDataGonder()
        {
            ...
            foreach (Etkinlik item in etkinlikListesi)
            {
                json += "{'Name' : " + item.EtkinlikAdi + ", 'City' : " + item.EtkinliginOlduguSehir + ", 'Date' : " + item.EtkinlikTarihi.ToString("yyyy.MM.dd") + ", 'ParticipantCount' : " + item.Kontenjan + ", 'Description' : " + item.Aciklama + " },";
            }
